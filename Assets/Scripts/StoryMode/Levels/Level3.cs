...
public class Level3 : MonoBehaviour
{
    ...
    void OnEnable() 
    {
        ...
        player.transform.position = new Vector3(-180, -128, 0);
        GameObject.FindGameObjectWithTag("MainCamera").transform.position = new Vector3(-180, -128, 0);
        player.GetComponent<PlayerStatsStory>().Initialize(new bool[]{true, false, false, false}, 1, 64);
        player.GetComponent<PlayerStatsStory>().enemiesLeft = 12;
        ...
        gg.center = new Vector3 (-126, -150, 0);
        gg.SetDimensions(250, 175, 0.5f);
    ...
    void Update() 
    {
        ...
        if (GameObject.FindGameObjectWithTag("Player").GetComponent<PlayerStatsStory>().enemiesLeft <= 0 && currentLine != 2)
        {
            currentLine = 2;
            nextLine = true;
        }
    ...
    {
        // Player leave area
        if (other.gameObject.tag == "Player" && currentLine >= 2)
        {
            Destroy(tempLevel);
            worldMap.LevelEnd(3);
        }
